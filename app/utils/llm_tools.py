...
def query_for_invoices(query: str) -> List[Document]:
    ...
    page_content: List[str] = [
        f"{doc.page_content}, invoice_cloud_location:{doc.metadata.get('gcp_blob_path')}"
        for doc in results
    ]
...
@tool
def send_downloaded_invoice_user(
    user_id: str,
    invoice_path: str,
    whats_app_file_caption: str,
    whats_app_file_name: str,
) -> str:
    """
    ...
    success, message = send_whatsapp_media(
        user_id, invoice_path, whats_app_file_caption, whats_app_file_name
    )
    if not success:
...
def send_message_reaction(user_id: str, message_id: str, reaction: str) -> str:
    ...
    """
    logger.info(
        f"Sending reaction '{reaction}' to message {message_id} from user {user_id}"
    )
